...
import { hospitais } from './planos';

var g_searchTerm = '';

class EscolheHospital extends React.Component {
  state: {
    dataSource: Object
  }
  ...
  constructor (props) {
    ...
    this.state = {
      dataSource: ds.cloneWithRows(hospitais)
    }
  }
  ...
  componentWillReceiveProps (newProps) {
    if (newProps.results) {
      this.setState({
        dataSource: this.state.dataSource.cloneWithRows(hospitais)
      })
    }
  }
...
export default connect(mapStateToProps, mapDispatchToProps)(EscolheHospital)

const _renderRow2 = (row) => {
  return (
    <FullButton onPress={NavigationActions.buscaPlanos} text={row} highlight={g_searchTerm} />
  )
}